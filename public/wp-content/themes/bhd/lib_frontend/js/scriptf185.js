jQuery(document).ready(function($){
    ...
    */
    ...
    if( data_object.id_rap != 0 ) {
            ...
            jQuery.post(data_object.ajax_url, {"action":"bhd_lichchieu_chonrap","cinema_id":cinema_id}, function(response) {
    ...
    */
    ...
    $(".bhd-chon-rap").change(function(e){
       ...
       if( movie_id == 0 ){
           ...
           $(".bhd-chon-showtime").html('<option value="0">'+data_object.text_chonxuatchieu+'</option>');
            ...
            jQuery.post(data_object.ajax_url, {"action":"bhd_search_movie","movie_id":movie_id,"cinema_id":cinema_id}, function(response) {
            var lists="";
            var dates ="";
            var i =0;
    ...
    $(".bhd-chon-phim").change(function(e){
       ...
       $(".bhd-chon-rap").html('<option value="0">'+data_object.text_loading+'</option>');
        $(".bhd-chon-showtime").html('<option value="0">'+data_object.text_chonxuatchieu+'</option>');
        $(".bhd-chon-date").html('<option value="0">'+data_object.text_chonngaychieu+'</option>');
        jQuery.post(data_object.ajax_url, {"action":"bhd_search_movies","movies_id":id}, function(response) {
            if(response == "" ){
    ...
    $(".bhd-lich-chieu-chon-phim li a").click(function(e){
        ...
        jQuery.post(data_object.ajax_url, {"action":"bhd_lichchieu_chonphim","movies_id":movies_id,"post_id":post}, function(response) {
            $(".loading-rap").addClass("hide");
    ...
    $(".bhd-phone-thanhvien").focusout(function(e){
        ...
        jQuery.post(data_object.ajax_url, {"action":"bhd_danky_thanhvien","phone":phone}, function(response) {
            if( response.check == 1 ) {
    ...
    $(".bhd-ul-tab-phim-home").click(function(e){
        var id = $(this).data("id");
        jQuery.post(data_object.ajax_url, {"action":"bhd_load_data_phim","id":id}, function(response) {
            return false;
        })
    })
            ...
            if ($(".bhd-container-scroll").height() <= $(window).scrollTop() + $(window).height()) {
                if( load_more == 1 ) {
                   ...
                   $(".cssload-container").removeClass("hide");
                    jQuery.post(data_object.ajax_url, {"action":"bhd_load_more","paged":paged,"post_type":post_type,"cat":cat}, function(response) {
                        $(".cssload-container").addClass("hide");
                        if( response != "end" ) {
                            load_more = 1;
                            $("#bhd_paged").val( parseInt(paged) + 1);
                            $(".list--news").append(response);
                            $(".list--news").isotope('destroy');														$(".list--news").bind("load",function(){								$(".list--news").isotope({									itemSelector: ".js__isotope_item"								})							})
                        }
                        return false;
                    })
                }
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if( data == "error" ) {
                    ...
                    $(".member--info .avatar img").attr("src",data);
                }
            },
            error: function(data){
                alert("error");
